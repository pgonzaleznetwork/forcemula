...
let isCPQRelationship = value => {
    ...
    return object.toUpperCase().startsWith('SBQQ__') && object.toUpperCase().endsWith('__R');
}
...
let mapCPQField = (value,originalObject) => {
    ...
    if(cpqMapping[originalObject]){

        let mappingsForObject = cpqMapping[originalObject];

        if(mappingsForObject[object]){
            newName = `${mappingsForObject[object]}.${field}`
        } 
    }
